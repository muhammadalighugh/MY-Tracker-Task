...
export default function HealthTracker() {
  const { collapsed, sidebarOpen } = useSidebar();

  return (
    <div
      className={`min-h-screen bg-gradient-to-br from-indigo-50 to-slate-50 flex flex-col items-center justify-center transition-all duration-300 ${
        sidebarOpen ? (collapsed ? 'lg:ml-20' : 'lg:ml-64') : 'ml-0'
      } p-4 sm:p-6 md:p-8`}
    >
      {/* Header Icon with Animation */}
      <div className="mb-6">
        <FaTools className="text-indigo-600 text-4xl sm:text-5xl md:text-6xl animate-pulse" />
      </div>

      {/* Title */}
      <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold text-slate-800 text-center mb-4">
        Health Tracker Under Development
      </h1>

      {/* Message */}
      <p className="text-base sm:text-lg md:text-xl text-slate-600 text-center max-w-md sm:max-w-lg md:max-w-2xl mb-6 px-2">
        We're actively building the Health Tracker feature to help you monitor your wellness goals. Stay tuned for exciting updates!
      </p>
      ...
      <div className="flex flex-col sm:flex-row items-center justify-center gap-2 sm:gap-3 mb-8">
        ...
        <p className="text-sm sm:text-base text-slate-700 text-center">
          For inquiries, contact our administration at{' '}
          <a
      ...
      <button
        ...
        className="inline-flex items-center px-4 sm:px-6 py-2 sm:py-3 bg-indigo-600 text-white text-sm sm:text-base font-semibold rounded-lg shadow-md hover:bg-indigo-700 transition-colors duration-300 focus:outline-none focus:ring-2 focus:ring-indigo-500"
      >
        <svg
      ...
      <div className="mt-8 flex justify-center gap-2 sm:gap-3">
        <div className="w-2 h-2 sm:w-3 sm:h-3 bg-indigo-200 rounded-full animate-bounce" style={{ animationDelay: '0s' }}></div>
        <div className="w-2 h-2 sm:w-3 sm:h-3 bg-indigo-300 rounded-full animate-bounce" style={{ animationDelay: '0.2s' }}></div>
