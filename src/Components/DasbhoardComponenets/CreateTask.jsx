    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      days.push(
        <div
          ...
          className={`h-10 flex items-center justify-center cursor-pointer rounded-lg text-sm font-medium transition-all duration-200 relative
            ...
            {task.categories && task.categories.length > 0 && (
              <div className="flex gap-1 flex-wrap">
                {task.categories.slice(0, 3).map((cat, idx) => (
                  <span key={idx} className="px-2 py-0.5 bg-white rounded-md text-xs border border-slate-200">
                    {cat}
                  </span>
                ))}
                {task.categories.length > 3 && (
                  <span className="px-2 py-0.5 bg-white rounded-md text-xs border border-slate-200">
        ...
        <div className="bg-gradient-to-r from-blue-50 to-blue-100 p-4 md:p-6 rounded-xl border border-blue-200">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xl md:text-2xl font-bold text-blue-900">{tasks.pending.length}</p>
            </div>
            <Clock className="w-6 h-6 md:w-8 md:h-8 text-blue-600" />
          </div>
        </div>
        <div className="bg-gradient-to-r from-red-50 to-red-100 p-4 md:p-6 rounded-xl border border-red-200">
          <div className="flex items-center justify-between">
            <div>
              <h3 className="text-xs md:text-sm font-medium text-red-800">Overdue</h3>
              <p className="text-xl md:text-2xl font-bold text-red-900">{tasks.overdue.length}</p>
            </div>
        ...
        <div className="bg-gradient-to-r from-green-50 to-green-100 p-4 md:p-6 rounded-xl border border-green-200">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xl md:text-2xl font-bold text-green-900">{tasks.history.length}</p>
            </div>
            <CheckCircle2 className="w-6 h-6 md:w-8 md:h-8 text-green-600" />
          </div>
        </div>
        <div className="bg-gradient-to-r from-purple-50 to-purple-100 p-4 md:p-6 rounded-xl border border-purple-200">
          <div className="flex items-center justify-between">
            <div>
              <h3 className="text-xs md:text-sm font-medium text-purple-800">Total Tasks</h3>
              <p className="text-xl md:text-2xl font-bold text-purple-900">{tasks.all.length}</p>
            </div>
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-4 md:mb-2">
          <div className="border-b border-slate-200 bg-white rounded-t-xl shadow-sm">
            <nav className="-mb-px flex overflow-x-auto px-4 md:px-6">
              {TABS.map((tab) => {
                const Icon = tab.icon;
                return (
                  <button
                    ...
                    className={`flex items-center gap-2 py-3 md:py-4 px-3 md:px-4 text-xs md:text-sm font-medium border-b-2 transition-colors duration-200 whitespace-nowrap ${
                      activeTab === tab.id
