...
// --- Tracker Item (Optimized for Per-Tracker Saving State) ---
  ...
  return (
    <div className="bg-white p-5 rounded-xl border border-slate-200/80 shadow-sm">
      <div className="flex items-center justify-between mb-3">
        ...
        {!isPremium && !isFreeTracker && <span className="bg-indigo-600 text-white text-xs font-semibold px-2 py-1 rounded-full">Pro</span>}
      </div>
      ...
      <label className="flex items-center cursor-pointer">
        <input type="checkbox" checked={isActive} onChange={() => onToggle(tracker.id)} className="hidden" disabled={isUpdating} />
...
const DigitalWatch = React.memo(({ tasks, onSaveTask, onDeleteTask }) => {
  ...
  const handleSaveSession = useCallback(() => {
    ...
    resetStopwatch();
  }, [taskName, stopwatchTime, resumingTaskId, onSaveTask, resetStopwatch]);
...
const TrackerManagerModal = React.memo(({ isOpen, onClose, activeTrackers, customTrackers, handleToggleTracker, handleCreateCustomTracker, isPremium, userDocRef }) => {
